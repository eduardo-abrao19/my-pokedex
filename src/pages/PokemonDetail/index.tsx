...
import { fetchPokemonDetail, fetchPokemonSpacies, type PokemonDetailResponse, type PokemonSpeciesResponse} from '../../services/pokeapi';
    ...
    async function loadPokemon() {
      try {
        ...
        const [detail, species] = await Promise.all([
          ...
          fetchPokemonSpacies(id, {signal: controller.signal}),
        ]);
        ...
        if ((e as Error).name !== 'AbortError') {
          setError('Nao carrga os dados do pokemon')
        }
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.typeContainer}>
          {pokemon.types.map(({ type }) => (
            <View key={type.name} style={[styles.typeBadge,{backgroundColor: TYPE_COLORS[type.name] }]}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.sectionText}>{description ?? 'Descrição indisponivel'}</Text>
      </View>
